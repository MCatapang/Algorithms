...
function cakes(recipe, available) {
    let countArr = [];
    for(let [rIngredient, rAmount] of Object.entries(recipe)) {
        let check = false;
        for(let [aIngredient, aAmount] of Object.entries(available)) {
            if(rIngredient == aIngredient && rAmount <= aAmount) {
                countArr.push(Math.floor(aAmount/rAmount));
                check = true;
            }
        }
        if(!check) {

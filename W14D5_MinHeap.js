...
class MinHeap {
   ...
   */
    extract() {
        if(this.heap.length == 1 && this.heap[0] == null) {
            return null;
        }
        let lastNum = this.heap.pop();
        this.heap[1] = lastNum;
        let i = 1;
        while(this.heap[i] > this.heap[i*2] || this.heap[i] > this.heap[i*2+1] ) {
            let leftChild = this.heap[i*2];
            let rightChild = this.heap[i*2+1];
            let smallerChild, indexOfSmallerChild;
            if(leftChild > rightChild) {
                smallerChild = rightChild;
                indexOfSmallerChild = i*2+1;
            } else {
                smallerChild = leftChild;
                indexOfSmallerChild = i*2;
            }
            if(this.heap[i] > smallerChild) {
                let holder = this.heap[i];
                this.heap[i] = smallerChild;
                this.heap[indexOfSmallerChild] = holder;

...
class DoublyLinkedList {
    ...
    /**
      ...
      */
    isPalindrome(startFirst = this.head, lastFirst = this.tail) {
        if(startFirst == lastFirst){
            return true;
        }
        if(startFirst.data == lastFirst.data){
            if(startFirst.next == lastFirst && lastFirst.prev == startFirst){
                return true;
            }
            return this.isPalindrome(startFirst.next, lastFirst.prev);
            }
        return false;
    }
    ...
    /**
      ...
      * @returns {boolean}
      */
    isNodeInLeftHalf(data) {
        let runner = this.head;
        let totalNodes = 0;
        let nodePlace = 0;
        while (runner) {
            totalNodes++;
            if (runner.data == data && nodePlace == 0) {
                nodePlace = totalNodes;
            }
            runner = runner.next;
        }
        return (nodePlace <= totalNodes / 2) ? true : false;
    }
...
console.log(unorderedList.isNodeInLeftHalf(-3));